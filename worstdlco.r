...
targets <- ddply(subset(tPFT, years >= yearlb), ~ PtID, summarize,
                 lowpdlco=worstobs(perc.DLCO.of.predicted))
...
yearly <- ddply(pft, ~ PtID, yearavgs, "perc.DLCO.of.predicted", yearlb)

require(reshape2)

yearly <- dcast(yearly, PtID ~ year, value.var="perc.DLCO.of.predicted")
...
m0 <- lm(lowpdlco ~ Y0, data=yearly)
m1 <- lm(lowpdlco ~ Y0 + Y1, data=yearly)
m2 <- lm(lowpdlco ~ Y0 + Y1 + Y2, data=yearly)
m3 <- lm(lowpdlco ~ Y0 + Y1 + Y2 + Y3, data=yearly)
...
residuals <- yearly[, c("PtID", "lowpdlco", "m0", "m1", "m2", "m3")]
residuals <- melt(residuals, id.vars=c("PtID", "lowpdlco"),
                  variable.name="model", value.name="prediction")
res2 <- melt(residuals, id.vars=c("PtID", "lowpdlco", "m0"),
                  variable.name="model", value.name="prediction")
residuals[["group"]] <- factor(grplbl(residuals$PtID), 0:3)
res2[["group"]] <- factor(grplbl(residuals$PtID), 0:3)
res2 <- transform(res2, baseline=lowpdlco - m0)
res2 <- transform(res2, extended=lowpdlco - prediction)
...
p <- ggplot(residuals, aes(x = group == 0, y = lowpdlco - prediction, color=group))
...
above15 <- subset(group3, model=="m0" & abs(lowpdlco-prediction) > 15)$PtID
p <- ggplot(subset(group3, PtID %in% above15), aes(x=years, y=abs(lowpdlco-prediction)))
